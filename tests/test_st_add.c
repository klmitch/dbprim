...
smat_entry_t *
__wrap__smat_alloc(void)
{
  return (smat_entry_t *)mock();
}

db_err_t
__wrap_ht_add(hash_table_t *table, hash_entry_t *entry, db_key_t *key)
{
...
db_err_t
__wrap_ht_remove(hash_table_t *table, hash_entry_t *entry)
{
  check_expected_ptr(table);
  check_expected_ptr(entry);
  return (db_err_t)mock();
...
main(void)
{
  ...
  return cmocka_run_group_tests_name("Test st_add.c", tests, 0, 0);
}
