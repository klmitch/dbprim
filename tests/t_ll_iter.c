...
main(int argc, char **argv)
{
  ...
  check_result(ll_iter(0, 0, 0, 0, 0), DB_ERR_BADARGS, "ll_iter_noargs",
	       "ll_iter() with no arguments", 0);
  check_result(ll_iter(&list[1], 0, check_iter, &itcheck, 0), DB_ERR_BADARGS,
	       "ll_iter_badlist", "ll_iter() with bad list", 0);
  check_result(ll_iter(&list[0], 0, 0, &itcheck, 0), DB_ERR_BADARGS,
	       "ll_iter_badfunc", "ll_iter() with bad function", 0);

  /* Now check to see if ll_iter() returns what the iter function returns */
  check_result(ll_iter(&list[0], 0, check_iter, &itcheck, 0), EINVAL,
	       "ll_iter_funcreturn",
  ...
  itcheck.elem_idx = 0;
  check_result(ll_iter(&list[0], 0, check_iter, &itcheck, 0), 0,
