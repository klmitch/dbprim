...
db_err_t
__wrap_rt_remove(rb_tree_t *tree, rb_node_t *node)
{
  check_expected_ptr(tree);
  check_expected_ptr(node);
...
db_err_t
__wrap_rt_add(rb_tree_t *tree, rb_node_t *node, db_key_t *key)
{
  check_expected_ptr(tree);
  check_expected_ptr(node);
  check_expected_ptr(key);
...
main(void)
{
  ...
  return cmocka_run_group_tests_name("Test rt_move.c", tests, 0, 0);
}
